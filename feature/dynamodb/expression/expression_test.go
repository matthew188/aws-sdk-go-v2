	...
	cases := []struct {
		...
		},
		{
			...
			expected: Expression{
				...
				valuesMap: map[string]types.AttributeValue{
					":0": &types.AttributeValueMemberN{Value: "5"},
					":1": &types.AttributeValueMemberN{Value: "6"},
					":2": &types.AttributeValueMemberN{Value: "5"},
...
func TestValues(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expected: map[string]types.AttributeValue{
				":0": &types.AttributeValueMemberL{Value: []types.AttributeValue{}},
			},
		},
		{
			...
			input: Builder{
				expressionMap: map[expressionType]treeBuilder{
					update: Name("key").Equal(Value(&types.AttributeValueMemberS{Value: "value"})),
				},
			},
			expected: map[string]types.AttributeValue{
				":0": &types.AttributeValueMemberS{Value: "value"},
			},
		},
		{
			name: "condition",
...
func TestAliasValue(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			str, err := c.input.aliasValue(types.AttributeValue(nil))
