...
//
// Kinesis Video Streams HLS sessions
// contain fragments in the fragmented MPEG-4 form (also called fMP4 or CMAF) or
// the MPEG-2 form (also called TS chunks, which the HLS specification also
...
// an encrypted session token) for the session's HLS master playlist (the root
// resource needed for streaming with HLS). Don't share or store this token where
// an unauthorized entity could access it. The token provides access to the content
// of the stream. Safeguard the token with the same measures that you would use
// with your AWS credentials. The media that is made available through the playlist
...
// made available in a streaming session, any fragments that don't contain the same
// codec private data cause an error to be returned when those different media
// fragments are loaded. Therefore, the codec private data should not change
// between fragments in a session. This also means that the session fails if the
...
// stream media. Data retrieved with this action is billable. For more information,
