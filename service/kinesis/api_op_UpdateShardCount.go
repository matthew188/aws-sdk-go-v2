...
// Streams returns immediately and sets the status of the stream to UPDATING. After
// the update is complete, Kinesis Data Streams sets the status of the stream back
// to ACTIVE. Depending on the size of the stream, the scaling action could take a
// few minutes to complete. You can continue to read and write data to your stream
// while its status is UPDATING. To update the shard count, Kinesis Data Streams
// performs splits or merges on individual shards. This can cause short-lived
// shards to be created, in addition to the final shards. These short-lived shards
// count towards your total shard limit for your account in the Region. When using
// this operation, we recommend that you specify a target shard count that is a
// multiple of 25% (25%, 50%, 75%, 100%). You can specify any target value within
