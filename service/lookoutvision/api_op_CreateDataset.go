...
// Creates a new dataset in an Amazon Lookout for Vision project. CreateDataset can
// create a training or a test dataset from a valid dataset source (DatasetSource).
// If you want a single dataset project, specify train for the value of
// DatasetType. To have a project with separate training and test datasets, call
// CreateDataset twice. On the first call, specify train for the value of
// DatasetType. On the second call, specify test for the value of DatasetType. This
// operation requires permissions to perform the lookoutvision:CreateDataset
// operation.
...
type CreateDatasetInput struct {
	...
	// the dataset. If you don't specify DatasetSource, an empty dataset is created and
	// the operation synchronously returns. Later, you can add JSON Lines by calling
	// UpdateDatasetEntries. If you specify a value for DataSource, the manifest at the
	// S3 location is validated and used to create the dataset. The call to
	// CreateDataset is asynchronous and might take a while to complete. To find out
