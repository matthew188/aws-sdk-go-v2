...
// and key state. For details, see Managing keys
// (https://docs.aws.amazon.com/kms/latest/developerguide/getting-started.html) in
// the Key Management Service Developer Guide Use the parameters of CreateKey to
// specify the type of KMS key, the source of its key material, its key policy,
...
// operation. For instructions, see Importing key material into multi-Region keys
...
// store using its cryptographic keys. KMS supports CloudHSM key stores
...
// store, use the ConnectCustomKeyStore operation. To find the ConnectionState, use
// the DescribeCustomKeyStores operation. To create a KMS key in a custom key
...
// (https://docs.aws.amazon.com/kms/latest/developerguide/keystore-cloudhsm.html),
// use the Origin parameter with a value of AWS_CLOUDHSM. The CloudHSM cluster that
// is associated with the custom key store must have at least two active HSMs in
// different Availability Zones in the Amazon Web Services Region. To create a KMS
...
type CreateKeyInput struct {
	...
	// creates the key material. To create a KMS key with no key material
	...
	// about importing key material into KMS, see Importing Key Material
	// (https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html) in
	// the Key Management Service Developer Guide. The EXTERNAL origin value is valid
	// only for symmetric KMS keys. To create a KMS key in an CloudHSM key store
	...
	// the CloudHSM key store. The KeySpec value must be SYMMETRIC_DEFAULT. To create a
	...
	// key. For details, see Requirements for a KMS key in an external key store
	// (https://docs.aws.amazon.com/create-xks-keys.html#xks-key-requirements) in the
	// Key Management Service Developer Guide. Each KMS key in an external key store is
	// associated two backing keys. One is key material that KMS generates. The other
	// is the external key specified by this parameter. When you use the KMS key in an
	...
	// the specified external key, a process known as double encryption. For details,
