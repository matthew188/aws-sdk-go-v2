...
// operation is part of the custom key stores
// (https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html)
...
// CloudHSM key store, see Troubleshooting an CloudHSM key store
// (https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html) in the
// Key Management Service Developer Guide. External key store When you connect an
...
// external key store proxy. When you connect to an external key store that uses
// VPC endpoint service connectivity, KMS establishes the networking elements that
// it needs to communicate with your external key manager via the external key
...
// disconnecting an external key store, see Troubleshooting an external key store
