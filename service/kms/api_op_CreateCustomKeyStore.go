...
// (https://docs.aws.amazon.com/cloudhsm/latest/userguide/clusters.html) and
// external key stores
...
// Web Services. This operation is part of the custom key stores
// (https://docs.aws.amazon.com/kms/latest/developerguide/custom-key-store-overview.html)
...
// VPC_ENDPOINT_SERVICE, specify the XksProxyVpcEndpointServiceName parameter. For
// help with failures, see Troubleshooting an external key store
// (https://docs.aws.amazon.com/kms/latest/developerguide/xks-troubleshooting.html)
// in the Key Management Service Developer Guide.
//
// For external key stores: Some
// external key managers provide a simpler method for creating an external key
// store. For details, see your external key manager documentation. When creating
// an external key store in the KMS console, you can upload a JSON-based proxy
// configuration file with the desired values. You cannot use a proxy configuration
// with the CreateCustomKeyStore operation. However, you can use the values in the
// file to help you determine the correct values for the CreateCustomKeyStore
// parameters. When the operation completes successfully, it returns the ID of the
// new custom key store. Before you can use your new custom key store, you need to
// use the ConnectCustomKeyStore operation to connect a new CloudHSM key store to
...
// settings are correct and then disconnect it until you are ready to use it. For
// help with failures, see Troubleshooting a custom key store
// (https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html) in the
...
type CreateCustomKeyStoreInput struct {
	...
	// Specifies the type of custom key store. The default value is AWS_CLOUDHSM. For a
	// custom key store backed by an CloudHSM cluster, omit the parameter or enter
	...
	// the password of the kmsuser crypto user (CU) account
	...
	// has two required elements: RawSecretAccessKey, a secret key, and AccessKeyId, a
	// unique identifier for the RawSecretAccessKey. For character requirements, see
	// XksProxyAuthenticationCredentialType. KMS uses this authentication credential to
	// sign requests to the external key store proxy on your behalf. This credential is
	// unrelated to Identity and Access Management (IAM) and Amazon Web Services
	...
	// help making this choice, see Choosing a connectivity option
	...
	// VPC endpoint service. For external key stores with PUBLIC_ENDPOINT connectivity,
	// this endpoint must be reachable before you create the custom key store. KMS
	// connects to the external key store proxy while creating the custom key store.
	// For external key stores with VPC_ENDPOINT_SERVICE connectivity, KMS connects
	// when you call the ConnectCustomKeyStore operation. The value of this parameter
