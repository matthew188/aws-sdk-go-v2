...
// Athena User Guide. This request does not execute the query but returns results.
// Use StartQueryExecution to run a query. To stream query results successfully,
// the IAM principal with permission to call GetQueryResults also must have
// permissions to the Amazon S3 GetObject action for the Athena query results
// location. IAM principals with permission to the Amazon S3 GetObject action for
// the query results location are able to retrieve query results from Amazon S3
// even if permission to the GetQueryResults action is denied. To restrict user or
