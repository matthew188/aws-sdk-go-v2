...
// both SOFT_DELETE and HARD_DELETE. For SOFT_DELETE (default), feature columns are
...
// specified in DeleteRecord should be set later than the EventTime of the existing
// record in the OnlineStore for that RecordIdentifer. If it is not, the deletion
// does not occur:
//
// * For SOFT_DELETE, the existing (undeleted) record remains in
// the OnlineStore, though the delete record marker is still written to the
// OfflineStore.
//
