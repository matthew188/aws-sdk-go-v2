...
// For more information, see AggregatedProfileTime
...
// them as one aggregated profile. If aggregated profiles do not exist for the full
// time range requested, then aggregated profiles for a smaller time range are
// returned. For example, if the requested time range is from 00:00 to 00:20, and
// the existing aggregated profiles are from 00:15 and 00:25, then the aggregated
...
type GetProfileInput struct {
	...
	// calls method B, which calls method C, which calls method D, then the depth is 4.
	// If the maxDepth is set to 2, then the aggregated profile contains
