...
// the IoT SiteWise User Guide. To identify an asset property, you must specify one
// of the following:
//
...
//
// With
// respect to Unix epoch time, IoT SiteWise accepts only TQVs that have a timestamp
// of no more than 7 days in the past and no more than 10 minutes in the future.
// IoT SiteWise rejects timestamps outside of the inclusive range of [-7 days, +10
// minutes] and returns a TimestampOutOfRangeException error. For each asset
// property, IoT SiteWise overwrites TQVs with duplicate timestamps unless the
// newer TQV has a different quality. For example, if you store a TQV {T1, GOOD,
// V1}, then storing {T1, GOOD, V2} replaces the existing TQV. IoT SiteWise
// authorizes access to each BatchPutAssetPropertyValue entry individually. For
// more information, see BatchPutAssetPropertyValue authorization
