...
func TestClient_HttpPayloadWithXmlNamespaceAndPrefix_awsRestxmlSerialize(t *testing.T) {
	cases := map[string]struct {
		...
		"HttpPayloadWithXmlNamespaceAndPrefix": {
			...
			ExpectMethod:  "PUT",
			ExpectURIPath: "/HttpPayloadWithXmlNamespaceAndPrefix",
			ExpectQuery:   []smithytesting.QueryItem{},
			ExpectHeader: http.Header{
			...
			BodyAssert: func(actual io.Reader) error {
				return smithytesting.CompareXMLReaderBytes(actual, []byte(`<PayloadWithXmlNamespaceAndPrefix xmlns:baz="http://foo.com">
			    <name>Phreddy</name>
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			result, err := client.HttpPayloadWithXmlNamespaceAndPrefix(context.Background(), c.Params)
			if err != nil {
...
func TestClient_HttpPayloadWithXmlNamespaceAndPrefix_awsRestxmlDeserialize(t *testing.T) {
	cases := map[string]struct {
		StatusCode    int
		Header        http.Header
		BodyMediaType string
		Body          []byte
		ExpectResult  *HttpPayloadWithXmlNamespaceAndPrefixOutput
	}{
		// Serializes a structure in the payload using a wrapper with an XML namespace
		"HttpPayloadWithXmlNamespaceAndPrefix": {
			StatusCode: 200,
			...
			BodyMediaType: "application/xml",
			Body: []byte(`<PayloadWithXmlNamespaceAndPrefix xmlns:baz="http://foo.com">
			    <name>Phreddy</name>
			</PayloadWithXmlNamespaceAndPrefix>`),
			ExpectResult: &HttpPayloadWithXmlNamespaceAndPrefixOutput{
				Nested: &types.PayloadWithXmlNamespaceAndPrefix{
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			var params HttpPayloadWithXmlNamespaceAndPrefixInput
			result, err := client.HttpPayloadWithXmlNamespaceAndPrefix(context.Background(), &params)
			if err != nil {
				t.Fatalf("expect nil err, got %v", err)
			}
			if result == nil {
