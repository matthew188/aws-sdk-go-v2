...
func TestClient_NestedXmlMaps_awsRestxmlSerialize(t *testing.T) {
	cases := map[string]struct {
		...
		"NestedXmlMapRequest": {
			Params: &NestedXmlMapsInput{
				NestedMap: map[string]map[string]types.FooEnum{
					"foo": {
						"bar": types.FooEnum("Bar"),
					},
				},
			},
			ExpectMethod:  "POST",
			ExpectURIPath: "/NestedXmlMaps",
			ExpectQuery:   []smithytesting.QueryItem{},
			ExpectHeader: http.Header{
		...
		// they're structure members, only the outer map is flat.
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			result, err := client.NestedXmlMaps(context.Background(), c.Params)
			if err != nil {
...
func TestClient_NestedXmlMaps_awsRestxmlDeserialize(t *testing.T) {
	cases := map[string]struct {
		StatusCode    int
		Header        http.Header
		BodyMediaType string
		...
		// Tests responses with nested flat maps. Since maps can only be flattened when
		// they're structure members, only the outer map is flat.
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			var params NestedXmlMapsInput
			result, err := client.NestedXmlMaps(context.Background(), &params)
			if err != nil {
				t.Fatalf("expect nil err, got %v", err)
			}
			if result == nil {
