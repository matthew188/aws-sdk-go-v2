...
func TestClient_StreamingTraitsWithMediaType_awsRestjson1Serialize(t *testing.T) {
	cases := map[string]struct {
		...
		"RestJsonStreamingTraitsWithMediaTypeWithBlob": {
			...
			ExpectMethod:  "POST",
			ExpectURIPath: "/StreamingTraitsWithMediaType",
			ExpectQuery:   []smithytesting.QueryItem{},
			ExpectHeader: http.Header{
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			result, err := client.StreamingTraitsWithMediaType(context.Background(), c.Params)
			if err != nil {
...
func TestClient_StreamingTraitsWithMediaType_awsRestjson1Deserialize(t *testing.T) {
	cases := map[string]struct {
		StatusCode    int
		Header        http.Header
		BodyMediaType string
		Body          []byte
		ExpectResult  *StreamingTraitsWithMediaTypeOutput
	}{
		// Serializes a blob in the HTTP payload with a content-type
		"RestJsonStreamingTraitsWithMediaTypeWithBlob": {
			StatusCode: 200,
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			var params StreamingTraitsWithMediaTypeInput
			result, err := client.StreamingTraitsWithMediaType(context.Background(), &params)
			if err != nil {
				t.Fatalf("expect nil err, got %v", err)
			}
			if result == nil {
