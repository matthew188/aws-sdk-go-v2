...
func TestClient_EndpointOperation_awsEc2querySerialize(t *testing.T) {
	cases := map[string]struct {
		...
		"Ec2QueryEndpointTrait": {
			...
			BodyAssert: func(actual io.Reader) error {
				return smithytesting.CompareURLFormReaderBytes(actual, []byte(`Action=EndpointOperation&Version=2020-01-08`))
			},
		},
	}
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			if name == "Ec2QueryEndpointTrait" {
				t.Skip("disabled test aws.protocoltests.ec2#AwsEc2 aws.protocoltests.ec2#EndpointOperation")
			}
